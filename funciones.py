...
def ctl_npuntos(parametros, archivo, cifras=6):
    identidad = str(parametros[0])
    for i in range(len(parametros)-1):
        identidad = identidad + '-' + str(parametros[i+1])
    
    n = int(len(parametros)/2)
    epsilons = parametros[0:n]
    puntos = parametros[n:len(parametros)]

    control = open(archivo, "r")
    cypris = open("cypris-" + identidad + "x.ctl" , "w")
    for i, lines in enumerate(control.readlines(), 0):
        if i == 11:
            for j in range(n): cypris.write( "(define-param eps" + str(j)  + " " + str(epsilons[j]) + ")\n" )
            cypris.write("\n")                  
            for j in range(n): cypris.write( "(define-param x" + str(j)  + " " + str(puntos[j]) + ")\n" )
        if i == 15:
            for j in range(n):
                if (j != (n-1)):
                    cypris.write( "      (if ( and (>= (vector3-x p) x" + str(j) + ") (< (vector3-x p) x" + str(j+1) + ") )\n" )
                    cypris.write( "        (index  (+ eps" + str(j) + " (* (/ (- eps" + str(j+1) + "  eps" + str(j) + " ) (- x" + str(j+1) + " x" + str(j) + "))  (- (vector3-x p) x" + str(j) + ") ) ) )\n" )
                if (j == (n-1)):
                    cypris.write( "        (index  (+ eps" + str(j-1) + " (* (/ (- eps" + str(j) + "  eps" + str(j-1) + " ) (- x" + str(j) + " x" + str(j-1) + "))  (- (vector3-x p) x" + str(j-1) + ") ) ) )\n" )
            for j in range(n-1):
                cypris.write( "      )\n" )
        else:
            cypris.write(lines)
...
# mantiene proporciones que se establescan en control
# se recomienda colocar material que se va a cambiar como 0.0 en control_balance
# (para mas de 2 materiales) y garantizar que los restantes sumen 1
# asi es mas facil ver las proporciones de antemano
def balance_proporcional_n(matriz,control_balance,material,cifras):
# material es el indice del material que va a cambiar iniciando en 0
    num_mats = int(len(matriz[0])/2)
    control_propor = control_balance[num_mats:len(control_balance)]
    factor_inv = 1.0/(1.0-control_propor[material])
    proporciones = []
    for j in range(num_mats):
        if j != material:
            proporciones.append(control_propor[j]*factor_inv)
        else:
            proporciones.append(1.0)    
    
    for i in range(len(matriz)):
        disponible = 1.0 - matriz[i][material+num_mats]
        for j in range(num_mats):
            if j != material: 
                matriz[i][j+num_mats] = round(disponible*proporciones[j],cifras)

    return matriz

def para_ee(matriz,cifras):
    for i in range(len(matriz)):
        matriz[i][1] = round((1.0 + matriz[i][1]*(matriz[i][0]-1.0)),cifras)
    return matriz

def para_aa(matriz,cifras):
    for i in range(len(matriz)):
        matriz[i][4] = round( (matriz[i][4]*(1.0-matriz[i][3])) ,cifras)
        matriz[i][5] = round( (1.0 - matriz[i][3] - matriz[i][4]) ,cifras)
    return matriz
...
def rutina(num_bandas=6,
    ...
    for i in range(len(puntos)):
        if (tipo[0] == 'num'):
            ...
            ctl_nmat(parametros[i], archivo_control, cifras=cifras+1)
        elif (tipo[0] == 'lineal'):
            ctl_lineal(parametros[i], archivo_control)       
        elif (tipo[0] == 'npuntos'):
            ctl_npuntos(parametros[i], archivo_control)       
